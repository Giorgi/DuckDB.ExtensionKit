...
internal sealed class MapVectorDataReader : VectorDataReaderBase
{
    ...
    {
        using var keyTypeLogical = NativeMethods.NativeMethods.LogicalType.DuckDBMapTypeKeyType(logicalColumnType);
        using var valueTypeLogical = NativeMethods.NativeMethods.LogicalType.DuckDBMapTypeValueType(logicalColumnType);

        var childVector = NativeMethods.NativeMethods.Vectors.DuckDBListVectorGetChild(vector);

        var keyVector = NativeMethods.NativeMethods.Vectors.DuckDBStructVectorGetChild(childVector, 0);
        var valueVector = NativeMethods.NativeMethods.Vectors.DuckDBStructVectorGetChild(childVector, 1);
