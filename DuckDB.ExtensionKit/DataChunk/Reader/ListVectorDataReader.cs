...
internal sealed class ListVectorDataReader : VectorDataReaderBase
{
    ...
    {
        using var childType = IsList ? NativeMethods.NativeMethods.LogicalType.DuckDBListTypeChildType(logicalColumnType) : NativeMethods.NativeMethods.LogicalType.DuckDBArrayTypeChildType(logicalColumnType);

        var childVector = IsList ? NativeMethods.NativeMethods.Vectors.DuckDBListVectorGetChild(vector) : NativeMethods.NativeMethods.Vectors.DuckDBArrayVectorGetChild(vector);

        arraySize = IsList ? 0 : NativeMethods.NativeMethods.LogicalType.DuckDBArrayVectorGetSize(logicalColumnType);
