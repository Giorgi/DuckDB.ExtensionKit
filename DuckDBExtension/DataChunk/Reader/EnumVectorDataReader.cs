...
internal sealed class EnumVectorDataReader : VectorDataReaderBase
{
    ...
    protected override T GetValidValue<T>(ulong offset, Type targetType)
    {
        ...
        unsafe T ToEnumOrString<TSource>(TSource enumValue) where TSource: unmanaged
        {
            if (typeof(T) == typeof(string))
            {
                var value = PointerExtensions.ToManagedString(NativeMethods.LogicalType.DuckDBEnumDictionaryValue(logicalType, (ulong)Convert.ToInt64(enumValue)));
    ...
    private unsafe object GetEnum(ulong offset, Type returnType)
    {
        ...
        {
            return PointerExtensions.ToManagedString(NativeMethods.LogicalType.DuckDBEnumDictionaryValue(logicalType, (ulong)enumValue));
        }
